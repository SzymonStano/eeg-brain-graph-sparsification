...
def run_training(sparsification_args):
    ...
    train_data = load_data_for_subjects(SUBJECTS_TRAIN, only_plv=True, scaling=True, add_self_loops_flag=True)
    val_data = load_data_for_subjects(SUBJECTS_VAL, only_plv=True, scaling=True, add_self_loops_flag=True)
    test_data = load_data_for_subjects(SUBJECTS_TEST, only_plv=True, scaling=True, add_self_loops_flag=True)
    ...
    model = GCN_EEG(num_node_features=num_features, num_classes=2, only_plv=True, **sparsification_args).to(DEVICE)

    mode = sparsification_args['mode']

    if mode == 'threshold':
        optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    else:
        # Faster learning rate for trainable thresholds in 'density' and 'top_k' methods
        sparsifier_params = []
        backbone_params = []

        for name, param in model.named_parameters():
            if "top_k_logit" in name:
                sparsifier_params.append(param)
            else:
                backbone_params.append(param)

        # # Definiujemy osobne paczki dla Adama
        optimizer = torch.optim.AdamW([
            {'params': backbone_params, 'lr': LR},              # Główny LR dla GNN i MLP
            {'params': sparsifier_params, 'lr': 0.005}               # AGRESYWNY LR dla wyboru percentyla (np. 0.01 lub 0.005)
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        # Logging trainable thresholds:
        if mode == 'threshold':
            print("Threshold: ", model.edge_sparsifier.threshold.item())
        elif mode == 'density':
            print("Density: ", model.edge_sparsifier.last_ratio)
        elif mode == 'top_k':
            print("Top-k: ", model.edge_sparsifier.last_k)
...
if __name__ == "__main__":
    ...
    modes = ['threshold', 'density', 'top_k_global', 'top_k_per_node']

    transformations = ['relu', 'sigmoid']

    for mode in modes:

        for gating in transformations:

            per_node = True if "per_node" in mode else False
            mode_correct = 'top_k' if 'top_k' in mode else mode 
            ...
            output_csv = ROOT_DIR / f"results/automatic_{mode}_{gating}.csv"

            print(
                f"\n=== RUNNING EXPERIMENT FOR {mode.upper()}: Gating: {gating} ==="
            )
            ...
            if kfold_cv:
                ...
                for fold, (train_val_idx, test_idx) in enumerate(kf.split(patients)):
                    ...
                    SAVE_MODEL_PATH = ROOT_DIR / "models" / f"best_model_automatic_{mode}_{gating}_fold_{fold + 1}.pth"
                    metrics = run_training(sparsification_args)
